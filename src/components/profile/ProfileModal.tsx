...
export const ProfileModal: React.FC<ProfileModalProps> = ({ isOpen, onClose }) => {
  const { currentUser, allUsers, switchUser, createCustomProfile, isDemoMode, theme, toggleTheme } = useAuth();
  const { accounts, resetDemoData, clearData } = useFinance();
  ...
  const handleCreateProfileSubmit = async (e: React.FormEvent) => {
    ...
    await createCustomProfile(newProfileName.trim(), parseFloat(newProfileBudget) || 0);
    setNewProfileName('');
    setShowCreateProfile(false);
  };

  const handleResetDataToZero = async () => {
    if (window.confirm('Reset all financial data to ₹0 clean state?')) {
      await clearData();
      onClose();
    }
  };
      ...
      <div className="w-full max-w-md theme-card rounded-3xl p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-[var(--card-divider)]">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-base font-black text-[var(--card-text-main)]">Settings & Appearance</h2>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <span className="text-[11px] text-[var(--card-text-sub)] block">
                {theme === 'dark' ? 'Dark Mode (Black Bg + Offwhite Cards)' : 'Light Mode (Offwhite Bg + Black Cards)'}
              </span>
            </div>
          </div>
        ...
        <div className="p-4 bg-black/5 dark:bg-black/5 border border-[var(--card-divider)] rounded-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-2xl bg-indigo-600/20 text-indigo-500 flex items-center justify-center text-base font-black">
              {currentUser.name.charAt(0)}
            </div>
            <div>
              <span className="text-sm font-bold text-[var(--card-text-main)] block">{currentUser.name}</span>
              <span className="text-xs text-[var(--card-text-sub)] block">{currentUser.email}</span>
            </div>
          </div>

          <span className="px-2.5 py-1 bg-indigo-500/20 text-indigo-500 text-[10px] uppercase font-bold rounded-lg font-mono">
            Active
          </span>
        </div>

        {/* Account Wallets & Physical Cash */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {accounts.map(acc => (
              <div key={acc.id} className="p-3 bg-black/5 dark:bg-black/5 border border-[var(--card-divider)] rounded-2xl">
                <span className="text-[11px] text-[var(--card-text-sub)] block truncate">{acc.name}</span>
                <span className="text-sm font-bold text-[var(--card-text-main)] font-mono-num mt-0.5 block">
        ...
        {showCreateProfile && (
          <form onSubmit={handleCreateProfileSubmit} className="p-4 bg-black/5 dark:bg-black/5 border border-[var(--card-divider)] rounded-2xl space-y-3">
            ...
            <div>
              <label className="text-[11px] text-[var(--card-text-sub)] block mb-1">Default Monthly Budget (₹)</label>
              <input
                type="number"
        ...
        {/* Reset all data to 0 */}
        <div className="pt-2 border-t border-[var(--card-divider)] space-y-2">
          <button
